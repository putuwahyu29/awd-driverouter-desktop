...
func (ws *WebServer) servePasswordPage(w http.ResponseWriter, name string, id string, hasError bool) {
	...
	errorHTML := ""
	if hasError {
		errorHTML = `<p id="error-msg" style="color: #ea4335; font-weight: 500; font-size: 13px; margin-top: -12px; margin-bottom: 16px;">Sandi salah! Silakan coba lagi.</p>`
	}

	fmt.Fprintf(w, `<!DOCTYPE html>
<html>
<head>
    <title id="page-title">Sandi Proteksi - Awd DriveRouter</title>
...
<body>
    <div class="card">
        ...
        <div class="brand">
            <img src="/logo.png" width="32" height="32" style="border-radius: 6px; object-fit: contain; vertical-align: middle;" />
            <span>Awd DriveRouter</span>
        </div>
    ...
    <script>
        ...
        const translations = {
            id: {
                title: "Sandi Proteksi - Awd DriveRouter",
            ...
            en: {
                title: "Password Protection - Awd DriveRouter",
...
`, html.EscapeString(name), errorHTML, id, html.EscapeString(name))
}
...
func (ws *WebServer) serveFilePage(w http.ResponseWriter, item *WebShareItem) {
	...
	if isImage {
        ...
        </div>`, item.ID, html.EscapeString(item.Name), html.EscapeString(item.Name), sizeStr)
	} else if isPdf {
        ...
        </div>`, html.EscapeString(item.Name), html.EscapeString(item.Name), sizeStr, item.ID)
	}
...
<html>
<head>
    <title>%s — Awd DriveRouter Preview</title>
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link href="https://fonts.googleapis.com/css2?family=Roboto:wght@400;500;700&display=swap" rel="stylesheet">
    <link rel="icon" type="image/png" href="/logo.png">
    <script>
    ...
    <header>
        <div class="header-left">
            ...
            <div class="file-title">
                <img src="/logo.png" width="22" height="22" style="border-radius: 4px; object-fit: contain; vertical-align: middle;" />
...
`, html.EscapeString(item.Name), backBtnHTML, html.EscapeString(item.Name), item.ID, previewHTML)
}

func (ws *WebServer) serveFolderPage(w http.ResponseWriter, item *WebShareItem) {
	w.Header().Set("Content-Type", "text/html; charset=utf-8")
	subFiles, _ := ws.App.database.GetFiles(item.VirtualID, false, "")

	var listRows strings.Builder
	for _, sf := range subFiles {
		sizeStr := formatBytes(sf.Size)
		...
		var iconHTML string
		if isImage {
			iconHTML = fmt.Sprintf(`<img src="/download/%s/%s?inline=1" alt="" loading="lazy" />`, item.ID, sf.ID)
		} else {
			iconHTML = iconSVG
		}
        ...
        `, item.ID, sf.ID, jsEscapedName, sizeStr, ext, sf.ID, iconCol, iconHTML, html.EscapeString(sf.Name), html.EscapeString(sf.Name), sizeStr, item.ID, sf.ID))
	}
...
<head>
    ...
    <style>
        ...
        .logo-text { font-size: 18px; font-weight: 600; color: var(--on-bg); margin-left: 8px; }
        ...
        @media (max-width: 768px) {
            ...
            .row { grid-template-columns: 24px 1fr auto 32px; gap: 8px; padding: 12px 10px; }
...
<body>
    <header>
        <a href="#" class="brand">
            <img src="/logo.png" width="26" height="26" style="border-radius: 6px; object-fit: contain; margin-right: 8px; vertical-align: middle;" />
            <span class="logo-text">Awd DriveRouter</span>
        </a>
        <div class="header-actions">
...
`, html.EscapeString(item.Name), html.EscapeString(item.Name), item.ID, listRows.String(), html.EscapeString(item.Name))
}

func (ws *WebServer) serve404Page(w http.ResponseWriter) {
	w.WriteHeader(http.StatusNotFound)
	w.Header().Set("Content-Type", "text/html; charset=utf-8")
	fmt.Fprintf(w, `<!DOCTYPE html>
<html>
<head>
    <title>Tautan Tidak Ditemukan - Awd DriveRouter</title>
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link href="https://fonts.googleapis.com/css2?family=Roboto:wght@400;500;700&display=swap" rel="stylesheet">
    <link rel="icon" type="image/png" href="/logo.png">
    <style>
        ...
        .card {
            ...
            border-radius: 24px; padding: 40px 32px; max-width: 400px; backdrop-filter: blur(16px);
        }
        h1 { font-size: 22px; color: #ea4335; margin: 0 0 12px 0; }
        p { color: #94a3b8; font-size: 14px; margin: 0; }
    </style>
</head>
<body>
    <div class="card">
        <h1>Tautan Kadaluarsa atau Tidak Ditemukan</h1>
        <p>Tautan berbagi web yang Anda coba akses tidak ada atau telah dihapus oleh pemiliknya.</p>
    </div>
</body>
</html>`)
}
