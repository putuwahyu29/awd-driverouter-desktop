...
func getStartupConfig() (dirPath string, filePath string, content string, err error) {
	...
	if runtime.GOOS == "darwin" {
		...
		filePath = filepath.Join(dirPath, "com.awd.driverouter.plist")
		content = fmt.Sprintf(`<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
    <key>Label</key>
    <string>com.awd.driverouter</string>
    ...
    <true/>
		...
		filePath = filepath.Join(dirPath, "AwdDriveRouter.desktop")
		content = fmt.Sprintf(`[Desktop Entry]
Type=Application
Version=1.0
Name=Awd DriveRouter
Comment=Awd DriveRouter Desktop Application
Exec="%s"
StartupNotify=false
...
func (a *App) SetStartup(enable bool) map[string]interface{} {
	...
	if enable {
		...
		_ = a.database.SaveSetting("auto_startup", "true")
	} else {
		if err := os.Remove(filePath); err != nil && !os.IsNotExist(err) {
			return map[string]interface{}{"success": false, "error": err.Error()}
		}
		_ = a.database.SaveSetting("auto_startup", "false")
	}
