...
!insertmacro MUI_PAGE_COMPONENTS # Component selection (shortcuts, startup, etc)
...
LangString UNINSTALL_CHECKBOX_TEXT ${LANG_INDONESIAN} "Hapus data pengguna (file konfigurasi, database, dan cache)"
...
LangString UNINSTALL_CHECKBOX_TEXT ${LANG_ENGLISH} "Delete user data (configuration files, database, and cache)"
...
OutFile "..\..\bin\awd-driverouter-${ARCH}-installer.exe" # Name of the installer's file.
...
Function checkAppRunning
  # Check if application executable exists on disk before lock test
  ...
  running:
    # Prompt user to kill running app automatically
  ...
  auto_close:
    ...
    Sleep 1000 # Wait 1 sec for process to terminate
    Goto check_lock
  done:
FunctionEnd

Function checkAndUninstallOldVersion
  # If previous installation is detected in a different path
  ${If} $OldUninstallPath != ""
  ${AndIf} $OldInstallDir != $INSTDIR
    MessageBox MB_YESNO|MB_ICONQUESTION "$(UNINSTALL_OLD_VER_CONFIRM)" IDNO skip
      DetailPrint "Uninstalling old version at: $OldInstallDir"
...
Function .onInit
   ...
   # Check if previously installed via registry
   SetRegView 64
...
# Component 1: Main Application (Mandatory)
...
# Component 2: Desktop Shortcut (Optional)
Section "$(COMP_SEC_DESKTOP)" SecDesktop
    !insertmacro wails.setShellContext
    CreateShortCut "$DESKTOP\${INFO_PRODUCTNAME}.lnk" "$INSTDIR\${PRODUCT_EXECUTABLE}"
SectionEnd

# Component 3: Start Menu Shortcut (Optional)
...
# Component 4: Run at Windows Startup (Optional)
...
Section "uninstall"
    ...
    ${If} $CheckboxState == 1
        # Delete user configuration data and sqlite database
        RMDir /r "$PROFILE\AppData\Roaming\driverouter"
        RMDir /r "$PROFILE\AppData\Local\driverouter"
